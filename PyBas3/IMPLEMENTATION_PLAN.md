...
```
PyBas3/
├── IMPLEMENTATION_PLAN.md          # This file (technical reference)
├── requirements.txt
...
### Purpose
Centralized definitions for inter-process communication protocols, data structures, and constants shared between Vision and Scoring modules.

### Structure
- **`protocols.py`**: Data structures (`ParticipantPose`, `PoseKeypoint`), constants (buffer names, sizes, landmark counts)
- **`shared_memory.py`**: Binary encoding/decoding functions for shared memory protocol
...
POSE_RECORD_SIZE = 573  # bytes per participant record
POSE_BUFFER_SIZE = 1719  # total buffer size (3 participants)
```

### Usage
Both Vision and Scoring modules import from `common`:
```python
from common.protocols import SHARED_MEMORY_BUFFER_NAME, ParticipantPose, POSE_BUFFER_SIZE
