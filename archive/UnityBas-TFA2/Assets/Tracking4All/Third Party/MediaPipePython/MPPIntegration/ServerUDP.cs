...
class ServerUDP
{
    ...
    private string[] eom = new string[] { "<EOM>" };
    private Queue<string> messageBuffer = new Queue<string>(); // Used when receive messages faster than we can process it.
    private int maxMessageBufferSize;
    private bool suppressWarnings;

    private int port;
    private static readonly byte[] aliveBuffer = new byte[] { 1 };
